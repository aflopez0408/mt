//+------------------------------------------------------------------+
//|                                      SMC_Strategy_EURUSD_EA.mq5  |
//|                        SMC POI Strategy - Expert Advisor for MT5  |
//|                     Optimizado para EURUSD - Smart Money (ICT)    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| INPUTS - Optimizados para EURUSD M5                                |
//+------------------------------------------------------------------+
input group "=== ESTRUCTURA DE MERCADO ==="
input int      InpSwingLen = 7;           // Longitud Swing (7 para EURUSD, menos ruido)
input bool     InpShowBOS = true;         // Mostrar BOS/CHoCH en grafico

input group "=== ORDER BLOCKS ==="
input int      InpMaxActiveOB = 5;        // Max OB activos (5 para EURUSD, mas zonas)
input double   InpOBSensitivity = 0.7;    // Sensibilidad OB (0.7x ATR para EURUSD)
...
input double   InpRiskReward = 2.5;       // Ratio R:R (2.5 para EURUSD, mas conservador)
input ENUM_SL_TYPE InpSLType = SL_OB_EDGE; // Tipo de Stop Loss
input double   InpSLATRMult = 1.8;        // Multiplicador ATR para SL (mayor para EURUSD)
input double   InpSLFixedPips = 20.0;     // SL Fijo (pips, menor que Oro)
input bool     InpUseTrailing = true;     // Usar Trailing Stop (si, para EURUSD)
input double   InpTrailATRMult = 1.5;     // Trailing ATR Mult (mas ajustado)
input int      InpMaxDailyTrades = 4;     // Max operaciones por dia (mas oportunidades)
input double   InpRiskPercent = 1.0;      // Riesgo % del balance por trade
input int      InpMagicNumber = 789012;   // Magic Number (diferente al de XAUUSD)
...
void OnDeinit(const int reason)
{
   ...
   Print("SMC POI Strategy EA EURUSD - Detenido");
}
...
//+------------------------------------------------------------------+
//| DETECCION DE ORDER BLOCKS (Sensibilidad ajustada para EURUSD)      |
//+------------------------------------------------------------------+
void DetectOrderBlocks()
{
   ...
   // Sensibilidad ajustada: EURUSD necesita menor umbral que XAUUSD
   double impulseThreshold = atr * InpOBSensitivity;
   
   bool isImpulsiveUp = (close1 > open1) && ((close1 - open1) > impulseThreshold) && (close1 > high2);
   bool isImpulsiveDown = (close1 < open1) && ((open1 - close1) > impulseThreshold) && (close1 < iLow(_Symbol, PERIOD_CURRENT, 2));
...
void DetectFVG()
{
   ...
   if(fvgBullBar > 30) { recentFVGBull = false; fvgBullBar = 0; }
   if(fvgBearBar > 0) fvgBearBar++;
   if(fvgBearBar > 30) { recentFVGBear = false; fvgBearBar = 0; }
}
         ...
         if(priceInOB && bosFilter && fvgFilter && trendFilter)
         {
            ...
            if(lots > 0 && sl > 0 && tp > 0)
            {
               if(ExecuteBuy(lots, sl, tp))
               {
                  obBull[i].active = false;
                  dailyTradeCount++;
                  Print("EURUSD LONG | Entry:", closePrice, " SL:", sl, " TP:", tp, " Lots:", lots);
               }
            }
            break;
         }
         ...
         if(priceInOB && bosFilter && fvgFilter && trendFilter)
         {
            ...
            if(lots > 0 && sl > 0 && tp > 0)
            {
               if(ExecuteSell(lots, sl, tp))
               {
                  obBear[i].active = false;
                  dailyTradeCount++;
                  Print("EURUSD SHORT | Entry:", closePrice, " SL:", sl, " TP:", tp, " Lots:", lots);
               }
            }
            break;
         }
...
double CalculateSL(bool isLong, double obEdge, double entryPrice, double atr)
{
   ...
   {
      if(isLong) sl = obEdge - (atr * 0.3);  // 0.3 para EURUSD (mas espacio)
      else       sl = obEdge + (atr * 0.3);
   }
